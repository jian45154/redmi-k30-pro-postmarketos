...
每个 boot 产物在 manifest 中声明且只能声明一种角色：

- `direct-ram-boot`：只允许在 bootloader fastboot 中执行临时 `fastboot boot`；
- `direct-persistent-boot`：只允许写入已解析的当前 boot slot；
- `copydown-fastbootd-boot`：只允许在 recovery fastbootd、`is-userspace=yes` 时写入 manifest 指定目标。

直接 pmbootstrap boot 与 copydown/bootshim 镜像不可互换。静态验证必须在任何手机操作前失败关闭。它检查产品名、产物角色、boot header v2 参数、镜像容量、GPT/4096 字节扇区解释、boot/root UUID 匹配、RNDIS 标识、OpenRC runlevel、关键二进制/脚本哈希、SSH 安全配置、专有文件清单和秘密扫描。
...
- 唯一设备且 `product=lmi`；
- bootloader 解锁；
- `is-userspace` 与 manifest 声明的 boot 路线一致；copydown 写入必须为 `yes`；
- 读取 `current-slot`、`slot-count`、`has-slot:boot` 和适用的 partition metadata，将 `boot` alias 展开为唯一实际目标（例如 `boot_a`）；
- 实际展开后的分区名、分区容量和 slot 状态与 manifest 相符；userdata 必须确认为非 slot 分区且目标容量足够；
- 电池电压至少 `3800 mV`；
- 操作文件的绝对路径、SHA-256、大小和 manifest 身份匹配；
- 目标分区只能是设计允许的 `boot` 或 `userdata`；
- 已发布或已上传 Draft 的候选镜像和已验证回滚路径仍可读取。

用户已授予持续操作授权；执行代理在授权范围内按自动安全门执行，不要求用户在线等待。该授权不绕过运行环境、操作系统或外部平台要求的权限审批，所需权限仍通过正式审批/提权接口申请。任何 fastboot 变量缺失或互相矛盾、身份不匹配、哈希变化、多设备、低电量、回滚失效或目标超出边界都会中止，而不是猜测。
...
binary-replay 在隔离的本地 APK 仓库中安装原始 APK 文件。每个文件先验证 D80 `SHA256SUMS`；因历史签名公钥已经失踪，只允许对这些精确哈希使用局部、显式的 untrusted 安装例外，不把例外写入全局 apk 信任，也不重签后继续沿用旧 APK 身份。manifest 同时记录原签名身份缺口和实际安装哈希。

源码阶段不声称恢复未知的 r9 配方。它从锁定的 LineageOS 提交、仓库 r8 配方/配置/补丁和 r9 APK 的静态内容审计建立新的 r10+：比较编译配置、Image 标识、合并 DTB 属性、包内容和启动行为，记录每项已知差异，然后重新执行完整 P1/P2 实机验收。只有新 r10+ 源码构建可以进入最终 P2-source/P3 镜像。
...
- 必须执行提权的 P1/P2/P3 操作只通过版本化 `lmi-rootctl` 子命令或一次性 OpenRC job 完成；参数、路径、服务、debugfs 读取范围、固件只读挂载和 mixer 上限都使用固定白名单，不接受任意 shell、任意命令或任意路径，也不给 `lmi` 用户通用免密 root shell。每次调用记录子命令、参数、调用用户、boot/image identity、时间、退出状态和脱敏输出。
...
私有仓库只提供访问控制，不产生复制或再分发许可。只有在用户确实拥有备份/上传权利、相应许可允许且 GitHub 条款允许时，专有输入才可进入私有 artifact 仓库。否则镜像保持 blob-free，并锁定可合法取得的 stock ROM URL、版本、完整包 SHA-256、提取脚本、所需文件路径及逐文件哈希。若合法输入无法形成可恢复闭环，P3 不得标为 `accepted`，并立即触发下游/主线路线重评估。
...
在第一个稳定阶段发布前，公开仓库以及实际承载完整镜像的私有仓库都启用 GitHub Release immutability。发布流程固定为：

1. 建立指向确切源码提交的 release-specific tag，并推送该 tag；
2. 使用已存在的 tag 创建 Draft，上传全部资产并完成下载复验；
3. 只在资产闭合后发布；发布后不再补充、替换或删除资产；
4. 验证 Release API 的 immutable 状态、tag 指向和每个 asset digest；
5. 执行 `gh release verify TAG` 和对每个本地下载资产执行 `gh release verify-asset TAG FILE`，保存 attestation 验证结果。

稳定阶段不依赖可移动 tag；删除 immutable release 后也不复用其 tag 名称。Draft 仍视为可变候选，不算防死档完成证据。

### 9.3 公开与私有边界

不包含不可再分发专有内容的镜像发布到当前公开仓库。P3 如果必须把可合法备份但不适合公开的 Xiaomi/Qualcomm/Cirrus/Focaltech blob 嵌入镜像，则：

- 完整镜像发布到用户账号下的私有 GitHub artifact 仓库；
- 公开仓库只记录源代码、脱敏 manifest、哈希和私有资产的逻辑身份；
- 不用加密公开附件规避许可证；
- 私有仓库创建和 Release 发布属于用户已经授权的目标范围，但仍必须先通过本设计的权利/许可检查。

## 10. 安全、授权与恢复

用户在 2026-07-19 明确授予目标范围内的持续操作授权，不要求用户持续在线；执行代理依照本设计门禁执行。该授权不替代运行环境、操作系统、GitHub 或其他平台自身的权限审批。授权覆盖：
...
2. userdata 写入按破坏性操作处理：候选镜像必须先存在于 GitHub Draft；还必须用证据证明当前 userdata 已是明确可丢弃的 pmOS 测试数据，或已经生成可读取、带 SHA-256、包含恢复命令并完成抽样恢复验证的 userdata/用户数据备份。Android ROM/boot 回滚不能替代个人数据备份。无法证明可丢弃且无法完成备份时禁止写 userdata。
