...
- Produces: `build_candidate(ctx: BuildContext) -> BuildResult`, where `BuildResult` contains absolute `boot_img`, `userdata_img`, `vmlinuz`, `initramfs`, `dtb_dir`, package manifest, world file, `sshd.pam` attestation, build log, and identity file paths.
...
- [ ] **Step 4: Implement the isolated two-pass replay**
  ...
  5. Run one `apk --no-network --allow-untrusted add "${replay_apks[@]}"` for the seven absolute replay paths. Verify no old key appeared under `/etc/apk/keys`, verify installed/package policy/world identity constraints are r139/r9/r10, discover the one installed `openssh-server-pam` version from the APK database, and add its exact `name=version` world pin before the final install.
  6. Run `install --no-fde --sector-size 4096 --no-sparse --password "$ephemeral_password"` without `--zap`; reject any change to the locked `openssh-server-pam` version.
  ...
  - Never copy the caller's staged-pmaports worktree or its `.git` directory. Reject replace refs plus every non-normal index flag (including `assume-unchanged` and `skip-worktree`), set `GIT_NO_REPLACE_OBJECTS=1`, create a local `--no-hardlinks --no-checkout` checkout of the pinned base object, and reconstruct the stage only from the isolated source commit's exact `artifacts/wsl-pmaports` overlay and initramfs patch. Compare the caller stage and reconstructed stage across their complete non-`.git` path/directory sets, regular-file or symlink bytes, and Git modes; the exact independently reconstructed manifest is the only allowed modification inventory. Reject every missing, extra, special, type-changed, mode-changed, or byte-changed entry, including ignored files, and stream hashes with before/after inode metadata checks.
  - Treat `ctx.repo` only as a local Git object source. Reject replace refs, require SHA-1 object format and the exact `ctx.source_commit`, create a local no-hardlink/no-checkout detached checkout, and compare every physical file/symlink byte, Git mode, and path to the pinned commit tree. Read every repository production input—including `files/lmi-p1`, both pmaports overlays, and the initramfs patch—only from this isolated checkout; dirty, ignored, assume-unchanged, or skip-worktree bytes in the caller worktree can never enter the candidate.
  - Accept only the pinned repository's tracked `pmbootstrap.py` blob. Make a fresh local no-hardlink clone inside the empty candidate, detach it at the pinned commit, verify the checkout is clean and the entrypoint blob matches, and invoke only that copy through `sys.executable -E -B` with global `--as-root` because the fixed CLI is deliberately run by the WSL `-u root` boundary. Rebuild the child environment from an explicit fixed allowlist only: root identity/home/shell, `C.UTF-8`, UTC, `/tmp`, `TERM=dumb`, `PATH=/usr/sbin:/usr/bin:/sbin:/bin`, and the four exact Git safety variables. Do not inherit proxy, agent, token, credential, certificate, `PYTHON*`, `LD_*`, `GIT_*`, shell-function, pmbootstrap, APK, ccache, XDG, or any other caller environment. Every internal Git child sets `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_TERMINAL_PROMPT=0`, and `GIT_NO_REPLACE_OBJECTS=1`; pmbootstrap receives that same minimal Git base so descendants cannot recover hostile state or unrelated host secrets. Every direct Git argv binds the resolved repository with exact `-c safe.directory=...` and disables hooks/fsmonitor, checkout filters fail closed before checkout, and no network clone is allowed.
  ...
  - Normalize `/etc/apk/world` to exact `name=version` pins for all seven replay packages and the installed `openssh-server-pam`, reject bare or conflicting entries after normalization and after the final install, and copy the verified pinned world into the image.
  - After the final install, require `/usr/sbin/sshd.pam` to be the unique file owned at that path by the single `openssh-server-pam` APK database record. Require package architecture `aarch64`, an executable regular non-symlink file under real `/usr/sbin` directories, and a complete little-endian 64-bit AArch64 ELF: full 64-byte header, executable/shared-object type, exact header/program-entry sizes, a bounded non-empty in-file program table, bounded/aligned load segments, and at least one executable load segment. Require byte identity with its APK database Q1/Q2 checksum. Emit canonical `sshd-pam.json` containing the exact package/version, APK checksum, size, and SHA-256; Task 4 must bind its mounted-image target to this attestation before target execution.
  ...
  - Reproduce the fixed pmbootstrap export contract exactly: ten standard absolute links are always present, known optional links may dangle and are then removed, and only exact dynamic `vmlinuz`, `initramfs`, and optional `initramfs-extra` names are allowed. Every real approved link must point to its exact in-candidate target and is atomically materialized as a one-link regular file; unknown, escaping, mismatched, or required-dangling links fail. Because fixed pmbootstrap does not recurse into `/boot/dtbs`, bind `deviceinfo_dtb=qcom/kona-v2.1-lmi`, create one controlled absolute link from the real rootfs `/boot/dtbs/qcom/kona-v2.1-lmi.dtb`, materialize it at `export/dtbs/qcom/kona-v2.1-lmi.dtb`, and hash the complete materialized inventory.

  Add focused RED tests for each correction before production changes: unlisted tracked/ignored pmaports changes and post-copy mutation; ignored/wrong pmbootstrap entrypoints, Python environment sanitization, hostile Git environment/global hooks, explicit safe-directory argv, and checkout-filter rejection; exact PAM/sudo/world/keyfile/takeover policy; missing/unowned/symlink/wrong-architecture/checksum-mismatched `sshd.pam`, an `openssh-server-pam` version change, and a conflicting world constraint; real-like absolute export links plus extra/escaping/dangling rejection and regular-file materialization.
...
- [ ] **Step 3: Implement exact structural checks**
  ...
  The verifier is invoked with fixed argv through `/mnt/c/Windows/System32/wsl.exe -d Ubuntu -u root -- ...`; its Python entry point immediately requires `geteuid()==0`. It validates every absolute tool path, package version, version output, and binary SHA-256 against `verification-tools.json` before consuming a candidate. The lock includes at least Python, util-linux `fdisk`/`losetup`/`mount`/`umount`/`unshare`, e2fsprogs `e2fsck`, the current Ubuntu qemu-user static-PIE binary `/usr/bin/qemu-aarch64`, coreutils `chroot`, and OpenSSH `ssh-keygen`. No interactive `sudo`, PATH lookup, shell string, or unlocked host tool is permitted.
