...
# Each built package revision has an immutable attestation. The r1-r3
# records remain available to the frozen D114 deployment chains.
BUILD_ATTESTATION_PATH = REPO / "config/lmi-weston-sixrow/build-attestation-r4.json"
...
COLUMNS = 11
# Rows 0 (Esc/arrows/Home/End/PgUp/PgDn/Bksp), 4 (modifiers, Shift leftmost)
# and 5 (page tag, always-available / : - . path keys and space) are fixed and
# identical on both pages; only rows 1-3 change when the [ABC/#&] tag is
# tapped.
FIXED_NAV_ROW = (
    ("Esc", 1), ("←", 1), ("↑", 1), ("↓", 1), ("→", 1),
    ("Hom", 1), ("End", 1), ("PgU", 1), ("PgD", 1), ("Bksp", 2),
)
FIXED_MODIFIER_ROW = (("Shift", 3), ("Tab", 2), ("Ctrl", 2), ("Enter", 4))
FIXED_BOTTOM_ROW = (
    ("ABC/#&", 2), ("/", 1), ("Space", 5), (":", 1), ("-", 1), (".", 1),
)
...
# Physical-keyboard muscle-memory pairs on the symbol page: shift on a digit
# or bracket key produces what a hardware keyboard would.
EXPECTED_SYMBOL_SHIFTED = {
    "1": "!", "2": "@", "3": "#", "4": "$", "5": "%",
    "6": "^", "7": "&", "8": "*", "9": "(", "0": ")",
    "[": "{", "]": "}", "\\": "|", "`": "~", "=": "+",
    "/": "?", "-": "_", ".": ">",
...
# symbol page (the / : - . path keys live on the shared bottom row, so they
# are one tap away on both pages).
...
def verify_layout(keyboard_source: str) -> None:
    ...
    symbol_shifted = {
        label: upper for row in symbol_rows for label, upper, _width in row
    }
    for key, expected in EXPECTED_SYMBOL_SHIFTED.items():
        if symbol_shifted.get(key) != expected:
            raise VerificationError(
                f"symbol page shifted mapping for {key!r} is not {expected!r}"
            )
...
def verify_build_attestation(*, require_artifact: bool = False) -> bool:
    ...
    if artifact["package_version"] != "14.0.2-r4":
        raise VerificationError("current build attestation is not the r4 modern layout")

    supersedes = attestation["supersedes"]
    if supersedes["status"] != "SUPERSEDED_R3_CENTER_SHIFT_NO_PERSISTENT_PATH_KEYS":
        raise VerificationError("superseded r3 artifact is not explicitly marked")
    previous_attestation = REPO / supersedes["attestation"]
    if digest(previous_attestation) != supersedes["attestation_sha256"]:
        raise VerificationError("superseded r3 attestation hash does not match")
    ...
    if old_artifact.exists() and digest(old_artifact) != supersedes["sha256"]:
        raise VerificationError("superseded r3 APK hash does not match its record")
    return True
