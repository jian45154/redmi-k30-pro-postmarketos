...
sanitize_public_image() {
	...
	: >"$machine_id" || fail "could not clear image machine-id"
	[[ "$(stat -Lc %a:%u:%g:%h:%s -- "$machine_id")" == 644:0:0:1:0 ]] ||
		fail "sanitized image machine-id metadata mismatch"
	[[ "$(sha256_of "$machine_id")" == e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855 ]] ||
		fail "sanitized image machine-id is not empty"
		...
		END {
			...
			for (path in modified_file) {
				...
				if (modified_file[path] == "machine-id" &&
					(stat_part(old_stat[path], 5) != 33 || stat_part(new_stat[path], 5) != 0 ||
