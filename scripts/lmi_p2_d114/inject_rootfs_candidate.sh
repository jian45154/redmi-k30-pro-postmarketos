...
readonly INPUT_BUILD_DIR="$REPO/private/lmi-p1/recovery/d110-d114/p2-d114-r2-most-complete-build-20260724"
readonly BUILD_DIR="$REPO/private/lmi-p1/recovery/d110-d114/p2-d114-r3-terminal-scroll-injected-20260724"
readonly RAW="$INPUT_BUILD_DIR/xiaomi-lmi-d114-r2-most-complete-userdata-20260724.normalized.img"
readonly RAW_SHA256=33067d6954e28b88b78a79a6ba0f994c1b6aff5e77a664b726e5dbb6e90084d8
readonly RAW_SIZE=3436183552
readonly SPARSE="$INPUT_BUILD_DIR/xiaomi-lmi-d114-r2-most-complete-userdata-20260724.android-sparse.img"
readonly SPARSE_SHA256=1315e3a06ddff42e91f930f01b16a62ab30ab3d4f490e8e8e40d0af89c657279
readonly SPARSE_SIZE=2269624624
...
readonly BASE_SHA256=5f351c9184fec53070886f3e9aa6a04178d2be8858ec2237b13af19e4a0e8cf6
readonly INPUT="$INPUT_BUILD_DIR/lmi-d114-rootfs-p2-candidate-20260724.ext4"
readonly INPUT_SHA256=d331433af3b7fdb78e42732a1d6b5530a5cf9e6a90a4f4e648f7a97aa696f790
...
readonly REPAIR_VERIFY_LOG_SHA256=b2256b9695e96bf57505a107edb2ca1581bcc307b70fc04997e1f016e936daf5
readonly REPAIR_LOG="$INPUT_BUILD_DIR/candidate-preinstall-e2fsck-repair.log"
readonly REPAIR_LOG_SHA256=b2256b9695e96bf57505a107edb2ca1581bcc307b70fc04997e1f016e936daf5
readonly REBUILD_LOCK="$REPO/config/lmi-p2-d114/candidate-rebuild-lock.json"
readonly REBUILD_LOCK_SHA256=80c4952c9f71eda4bb49d72facd451630927a53a5550103caffe9e818469a7a2
readonly REBUILD_LOCK_SCHEMA=lmi-p2-d114-candidate-rebuild-lock/v1
readonly OUTPUT_BUNDLE="$BUILD_DIR/lmi-d114-rootfs-p2-r3-terminal-scroll-injected-20260724.bundle"
readonly OUTPUT="$OUTPUT_BUNDLE/rootfs.ext4"
readonly ATTESTATION="$OUTPUT_BUNDLE/attestation.json"
readonly P2_APK="$REPO/private/lmi-p1/recovery/d110-d114/p2-d114-r3-userdata-integration-20260724/run2-device-xiaomi-lmi-terminal-0.1.0-r3.apk"
readonly P2_APK_SHA256=55d0bfe3353fa34559b21f11319da87894d52efc94a7275c7a72b01d8816aeb6
readonly P2_APK_SIZE=8777
readonly P2_APK_CHECKSUM=Q1pN/EowCTsV6u3Xh4OibqPmxx0j8=
readonly SIXROW_APK="$REPO/private/lmi-p1/recovery/d110-d114/p2-d114-r3-terminal-scroll-build-20260724/lmi-weston-sixrow-clients-14.0.2-r3.apk"
readonly SIXROW_APK_SHA256=9310963550ac26e28b187a3a1b0a202f9e917c8103a30e0a1dbaaf182626d010
readonly SIXROW_APK_SIZE=122316
readonly SIXROW_APK_CHECKSUM=Q1odSg8XdVLjAQRis4fXvp4hNFWCw=
...
readonly P2_BUILD_ATTESTATION_SHA256=3889cce0de7d90e70fddcc7bd34b1d791d0b6878717b0b9667fcca1b684d0e48
readonly SIXROW_BUILD_ATTESTATION="$REPO/config/lmi-weston-sixrow/build-attestation-r3.json"
readonly SIXROW_BUILD_ATTESTATION_SHA256=b8666eb60e60253cdfe671b859c1563ba80652022fedb3b1e80da7c10fd50742
...
readonly MACHINE_ID=1835a845d0bb85b283be20a5fd1c18a4
readonly MACHINE_ID_SHA256=1ee35c54671c7279d22a90441490b2f25c61d9f652bb7defb28ce9f56d9b7c43
...
sanitize_public_image() {
	...
	# Write a fixed VALID 32-hex machine-id. Empty (truncate) or absent (delete)
	# both make dbus refuse to start on this OpenRC image (the machine-id
	# openrc service that would regenerate an absent id is not enabled in any
	# runlevel), which cascades into elogind and the session gate -> black
	# screen. A populated valid id is the hardware-validated fix. This bakes a
	# fixed id (fine for a single-device hardware test); a release image should
	# instead enable /etc/init.d/machine-id in the boot runlevel to regenerate a
	# unique id on first boot.
	...
	printf '%s\n' "$MACHINE_ID" >"$machine_id" || fail "could not write image machine-id"
	[[ "$(stat -Lc %a:%u:%g:%h:%s -- "$machine_id")" == 644:0:0:1:33 ]] ||
		fail "written image machine-id metadata mismatch"
	[[ "$(sha256_of "$machine_id")" == "$MACHINE_ID_SHA256" ]] ||
		fail "written image machine-id digest mismatch"
...
verify_full_delta_fields() {
	...
	awk -F'|' -v before_file="$before" -v after_file="$after" '
		BEGIN {
			...
			add_file["/usr/libexec/lmi-p2-d114/session"]="755|15645|ea7ec5334bbd49212c38ff1b12e3c87cd785c7786018d06ac1e504791d16dc5a"
			...
			add_file["/usr/libexec/lmi-p2-d114/weston-terminal-sixrow"]="755|200960|ed3cda9c8dcdcd197385d125c84e3fdaa0429790b1b166d62f6206c4f853cf24"
			...
			modified_file["/etc/machine-id"]="machine-id"
		...
		END {
			...
			for (path in modified_file) {
				...
				if (modified_file[path] == "machine-id" &&
					(stat_part(old_stat[path], 5) != 33 || stat_part(new_stat[path], 5) != 33 ||
					 new_payload[path] != "1ee35c54671c7279d22a90441490b2f25c61d9f652bb7defb28ce9f56d9b7c43")) bad=1
...
validate_p2_installed_record() {
	...
	awk -v output="$record" -v package_checksum="$P2_APK_CHECKSUM" '
		BEGIN {
			...
			expected_value["V"]="0.1.0-r3"
			expected_value["A"]="noarch"
			expected_value["S"]="8777"
			...
			expected_value["c"]="uncommitted-p2-d114-source-lock-v5"
			expected_D="device-xiaomi-lmi=1-r144 greetd=0.10.3-r11 greetd-openrc=0.10.3-r11 greetd-phrog=0.53.0-r0 libseat=0.9.3-r1 libweston=14.0.2-r5 linux-xiaomi-lmi=4.19.325-r15 lmi-weston-sixrow-clients=14.0.2-r3 openrc=0.63.2-r0 seatd=0.9.3-r1 seatd-openrc=0.9.3-r1 weston=14.0.2-r5 weston-backend-drm=14.0.2-r5 weston-shell-desktop=14.0.2-r5 weston-terminal=14.0.2-r5 /bin/sh"
			...
			expected_dep["lmi-weston-sixrow-clients=14.0.2-r3"]=1
			...
			expected_file["usr/libexec/lmi-p2-d114/session"]="Q1a89yQInpgn+2tVL+RjJU8dNxhas="
...
validate_sixrow_installed_record() {
	...
	awk -v output="$record" -v package_checksum="$SIXROW_APK_CHECKSUM" '
		BEGIN {
			...
			expected_value["V"]="14.0.2-r3"
			expected_value["A"]="aarch64"
			expected_value["S"]="122316"
			...
			expected_value["t"]="1784851200"
			expected_value["c"]="uncommitted-terminal-touch-scrollback-r3"
			...
			expected_file["usr/libexec/lmi-p2-d114/weston-terminal-sixrow"]="Q1NWFVkiRYpn0e4jIrHdpsPkRavKg="
...
verify_scripts_delta() {
	...
	find "$final_dir" -xdev -mindepth 1 -maxdepth 1 -type f -name 'device-xiaomi-lmi-terminal-0.1.0-r3.*' -print | sort >"$target_list"
...
main() {
	...
	verify_image_file /usr/libexec/lmi-p2-d114/session 755 ea7ec5334bbd49212c38ff1b12e3c87cd785c7786018d06ac1e504791d16dc5a
	...
	verify_image_file /usr/libexec/lmi-p2-d114/weston-terminal-sixrow 755 ed3cda9c8dcdcd197385d125c84e3fdaa0429790b1b166d62f6206c4f853cf24
