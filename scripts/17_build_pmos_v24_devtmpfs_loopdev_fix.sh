...
source_boot=${SOURCE_BOOT:-$repo/artifacts/images/pmos-lmi-normalboot-v24-devtmpfs-20260623.img}
...
out_img=${OUT_IMG:-$repo/artifacts/images/pmos-lmi-normalboot-v24-devtmpfs-loopdevfix-currentuserdata-20260623.img}
out_manifest=${OUT_MANIFEST:-$repo/artifacts/images/pmos-lmi-normalboot-v24-devtmpfs-loopdevfix-currentuserdata-20260623.manifest}

for file in "$source_boot" "$unpack_bootimg" "$mkbootimg"; do
	[ -f "$file" ] || { echo "missing input: $file" >&2; exit 1; }
done

work=$(mktemp -d /tmp/lmi-pmos-v24-build.XXXXXX)
cleanup() {
	rm -rf -- "$work"
}
trap cleanup EXIT
...
grep -Fq 'fdisk $_lmi_fdisk_block_size -l "$partition"' \
...
(
	...
	find . -print0 | LC_ALL=C sort -z | cpio --null -o --format=newc --owner=0:0 --reproducible --quiet | gzip -9 > "$work/normal-devtmpfs-loopdevfix-ramdisk.gz"
)

# Current userdata on the phone was written from the v22-compatible image.
# Keep these UUIDs for RAM-only validation without rewriting userdata.
...
python3 "$mkbootimg" \
	...
	--ramdisk "$work/normal-devtmpfs-loopdevfix-ramdisk.gz" \
	--dtb "$work/unpack/dtb" \
	--pagesize 0x00001000 \
	--base 0x00000000 \
...

{
	...
	echo "devtmpfs_loopdevfix_ramdisk_sha256=$(sha256sum "$work/normal-devtmpfs-loopdevfix-ramdisk.gz" | cut -d' ' -f1)"
