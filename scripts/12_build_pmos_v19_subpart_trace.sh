...
include=$repo/scripts/pmos_http_subpart_trace.inc
out_img=${OUT_IMG:-$repo/artifacts/images/pmos-lmi-subpart-trace-v19-20260623.img}
out_manifest=${OUT_MANIFEST:-$repo/artifacts/images/pmos-lmi-subpart-trace-v19-20260623.manifest}

for file in "$source_boot" "$unpack_bootimg" "$mkbootimg" "$include"; do
	[ -f "$file" ] || { echo "missing input: $file" >&2; exit 1; }
done

work=$(mktemp -d /tmp/lmi-pmos-v19-build.XXXXXX)
cleanup() {
	rm -rf -- "$work"
}
trap cleanup EXIT
...
(
	...
	find . -print0 | LC_ALL=C sort -z | cpio --null -o --format=newc --owner=0:0 --reproducible --quiet | gzip -9 > "$work/subpart-trace-ramdisk.gz"
)
...
python3 "$mkbootimg" \
	...
	--ramdisk "$work/subpart-trace-ramdisk.gz" \
	--dtb "$work/unpack/dtb" \
	--pagesize 0x00001000 \
	--base 0x00000000 \
...

{
	...
	echo "subpart_trace_ramdisk_sha256=$(sha256sum "$work/subpart-trace-ramdisk.gz" | cut -d' ' -f1)"
